...
/**
 * Traverse tree with depth information passed to the callback.
 *
 * Note: this function was not in Windows original implentation. 
 * Added to be able to pretty print a tree structure.
 * @author Gerwert
 **/
int tree_traverse_preorder_wd(struct tree_t *tree, tree_traverser_wd_fn_t tvr)
{
	assert(tree);
	assert(tvr);
	if (tree->root)
		return tree_traverse_subtree_preorder_wd(tree->root, tvr, 0);
	return 0;
}
...
/* Traverse sub tree with depth information */
int tree_traverse_subtree_preorder_wd(struct tree_node_t *node, tree_traverser_wd_fn_t tvr, int depth)
{
	int ret;
	struct tree_node_t *curr;
	assert(node);
	...
	while (curr) {
		/* for consistency with tree_traverse_subtree_postorder(), remember next node */
		struct tree_node_t *next = curr->next;
		ret = tree_traverse_subtree_preorder_wd(curr, tvr, depth + 1);
		if (ret) return ret;
		curr = next;
	}
